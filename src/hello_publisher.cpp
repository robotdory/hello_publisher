...
HelloPublisher::HelloPublisher(const std::string & node_name)
: Node(node_name)
{
  ...
  hello_publisher_ = create_publisher<std_msgs::msg::String>("/hello", qos_profile);

  heartbeat_publisher_ = create_publisher<std_msgs::msg::Bool>("/heartbeat", qos_profile);

  publish_info_timer_ =
    this->create_wall_timer(500ms, std::bind(&HelloPublisher::publish_info, this));
}
